...
export const DevlogPage = ({DEVLOG_API}: Props) => {
  ...
  const load_devlog = (target: string | null) => {
    ...
    setLoading(true);
    getImages(DEVLOG_API, target).then((log)=>{
  ...
  useEffect(()=>{
    ...
    //max number of
    get_num_logs(DEVLOG_API).then((result)=>{
...
async function fetch_devlog(api:string, id: string): Promise<{[name: string]: any}> {
  let log = {};

  await fetch(`${api}/log/${id}`).then(async (result)=>{
    if(result?.ok) {
...
async function get_num_logs(api:string): Promise<number> {
  let num_logs = 0;

  await fetch(`${api}/log_count`).then(async (resp)=>{
    if(resp?.ok) {
